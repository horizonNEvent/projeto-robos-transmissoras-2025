...
export default function IePublicManager({ onLog }) {
    ...
    const fetchTargets = async () => {
        setLoading(true)
        try {
            const res = await axios.get(`${API_URL}/ie-public/targets`)
            setTargets(res.data)
        } catch (err) {
            console.error(err)
            onLog && onLog(`Erro ao buscar alvos WebIE Public: ${err.message}`)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleAdd = async () => {
        ...
        try {
            await axios.post(`${API_URL}/ie-public/targets`, {
                codigo_ons: newCode,
                nome: newName,
                ativo: true
            })
            ...
            onLog && onLog(`Alvo WebIE Public ${newName} adicionado.`)
        } catch (err) {
    ...
    const handleToggle = async (id) => {
        try {
            await axios.put(`${API_URL}/ie-public/targets/${id}/toggle`)
            fetchTargets()
        } catch (err) { console.error(err) }
    }
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await axios.delete(`${API_URL}/ie-public/targets/${id}`)
            fetchTargets()
            onLog && onLog(`Alvo WebIE Public removido.`)
        } catch (err) { console.error(err) }
    }
    ...
    return (
        <div className="card">
            <h3>Gerenciador de Alvos (WebIEPublic)</h3>
            <p style={{ color: '#64748b', fontSize: '0.9rem' }}>
                Defina quais transmissoras o robô WebIEPublic deve baixar no portal ISA/WebIE.
                Apenas alvos marcados como <strong>ATIVO</strong> entram na execução.
            </p>

            <div style={{ background: 'rgba(255,255,255,0.05)', padding: '1rem', borderRadius: '8px', marginBottom: '1rem', border: '1px dashed #475569' }}>
                <h4 style={{ margin: '0 0 1rem 0' }}>Adicionar transmissora à lista WebIE Public</h4>

                <div style={{ marginBottom: '1rem' }}>
                    <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8' }}>Buscar na base de transmissoras (opcional)</label>
                    <input
                        placeholder="Nome ou código ONS..."
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                    ...
                    <div style={{ flex: 3 }}>
                        <label style={{ display: 'block', fontSize: '0.8rem', color: '#94a3b8' }}>Nome *</label>
                        <input
                            value={newName}
                        ...
                        {targets.length === 0 && !loading && (
                            <tr>
                                <td colSpan={4} style={{ textAlign: 'center', padding: '2rem', color: '#64748b' }}>
                                    Nenhuma transmissora na lista WebIE Public. Adicione acima.
                                </td>
                            </tr>
                        )}
                    </tbody>
