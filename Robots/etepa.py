...
class EtepaBot:
    def __init__(self, user_code):
        ...
        self.base_url = "https://etepatransmissora.com.br"
        self.session = requests.Session()
        # Header setup mimicking browser interaction
    ...
    def login(self):
        ...
        Logic based on analysis of https://etepatransmissora.com.br/ js/main.js and index HTML.
        It sends 'cUser' (ONS Code). 'cPwd' is sent but appears unused/empty in typical flows analyzed.
        """
        url = f"{self.base_url}/login.php"
        payload = {
            "acao": "VLDUSER",
            "cUser": self.user_code,
            "cPwd": "" # Sending empty password as seen in script logic analysis where input might be hidden
        }
        
        print(f"Logging in to Etepa with user code: {self.user_code}...")
        ...
        try:
            ...
            # Fallback
            print("Warning: Could not decode JSON directly")
            import json
    ...
    def run(self, output_dir="downloads_etepa", target_date=None):
        ...
        for doc in documents_to_process:
            ...
            # .trim() logic seen in Etepa JS
            danfe_num = doc['danfe'].strip()
            ...
            # URL Construction based on JS analysis
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Etepa Transmissora Bot")
    ...
    if not raw_codes:
        parser.error("You must provide either --user or --agente with the code")
