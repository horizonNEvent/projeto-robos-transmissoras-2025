...
class GrupoArgoRobot(BaseRobot):
    """
    Robô para Grupo Argo / Argo Energia (via SigetPlus Company 39).
    URL: https://sys.sigetplus.com.br/cobranca/company/39/invoices
    Autenticação: Via query param ?agent=CODE.
    """

    def __init__(self):
        super().__init__("grupo_argo")
        self.base_url = "https://sys.sigetplus.com.br/cobranca/company/39/invoices"
    ...
    def run(self):
        ...
        for agente_ons in lista_agentes:
            self.logger.info(
                f"Iniciando Grupo Argo para ONS {agente_ons} - "
                f"Competência: {competencia_str or 'Mais Recente/Todos'}"
            )
            ...
            while True:
                ...
                if not linhas or (len(linhas) == 1 and "Nenhum" in linhas[0].text):
                    if pagina == 1:
                        self.logger.warning(f"Nenhum registro encontrado para Agente {agente_ons}.")
                    break
                ...
                for linha in linhas:
                    ...
                    if xml_link:
                        url = (
                            f"https://sys.sigetplus.com.br{xml_link['href']}"
                            if xml_link['href'].startswith('/')
                            else xml_link['href']
                        )
                    ...
                    if danfe_link:
                        url = (
                            f"https://sys.sigetplus.com.br{danfe_link['href']}"
                            if danfe_link['href'].startswith('/')
                            else danfe_link['href']
                        )
                    ...
                    for i, b_link in enumerate(boleto_links, 1):
                        if b_link.get('href'):
                            url = (
                                f"https://sys.sigetplus.com.br{b_link['href']}"
                                if b_link['href'].startswith('/')
                                else b_link['href']
                            )
                            self.baixar_arquivo(
                                url,
                                os.path.join(t_dir, f"BOLETO_{nome_base}_{i}.pdf"),
                                is_boleto=True,
                            )
            ...
            self.logger.info(f"Finalizado Agente {agente_ons}. Total baixado: {total_baixado}")

if __name__ == "__main__":
    robot = GrupoArgoRobot()
    robot.run()
