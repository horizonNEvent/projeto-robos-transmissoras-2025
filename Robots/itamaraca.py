...
def get_transmissora_data(agent_code):
    ...
    # Este endpoint parece ser específico para a Itamaracá (1307 no Siget)
    endpoint = '/cobranca/transmitter/1307/invoices'
...
def process_agent(agent_code, nome_ons, empresa_nome, base_path):
    ...
    # Cria o caminho da pasta: Raiz / Empresa / CodigoONS
...
def main():
    ...
    if not empresas:
        print("Nenhuma empresa configurada para processar.")
        return

    # Processa todos os códigos a partir do empresas.json
    for empresa_nome, mapping in empresas.items():
        print(f"\nIniciando grupo: {empresa_nome}")
