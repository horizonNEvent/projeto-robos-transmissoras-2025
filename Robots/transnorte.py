...
class TransnorteRobot(BaseRobot):
    
    def __init__(self):
        super().__init__("transnorte")
    ...
    # --- ALUPAR / TRANSNORTE Logic ---
    def processar_alupar(self, agent, output_dir):
        # TRANSNORTE -> Portal Alupar
        LOGIN_URL = "https://faturas.alupar.com.br:8090/Fatura/Emissao/26"
        BASE_DOMAIN = "https://faturas.alupar.com.br:8090"
        
        self.logger.info(f"[ALUPAR-TRANSNORTE] Tentando conexão Agent {agent}...")
        ...
        try:
            ...
            if resp.status_code != 200 or "erro" in resp.text.lower():
                self.logger.warning(f"[ALUPAR-TRANSNORTE] Login falhou ou retornou erro para {agent}.")
                return

            soup = BeautifulSoup(resp.text, 'html.parser')
            table = soup.find('table')
            if not table:
                self.logger.info(f"[ALUPAR-TRANSNORTE] Nenhuma tabela encontrada.")
                return
            ...
            self.logger.info(f"[ALUPAR-TRANSNORTE] Processando {len(to_process)} documentos de {target_month}/{target_year}")
            ...
            self.logger.error(f"[ALUPAR-TRANSNORTE] Erro {agent}: {e}")

    def run(self):
        self.logger.info("Iniciando Transnorte (Alupar)...")
