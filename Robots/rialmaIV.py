...
class RialmaIVRobot(BaseRobot):
    
    def __init__(self):
        super().__init__("rialmaiv")
    ...
    def processar_alupar(self, agent, output_dir):
        ...
        try:
            ...
            if resp.status_code != 200 or "erro" in resp.text.lower():
                self.logger.warning(f"[ALUPAR-TBO] Login falhou ou retornou erro para {agent}.")
                return

            soup = BeautifulSoup(resp.text, 'html.parser')
            table = soup.find('table')
            if not table:
                self.logger.info(f"[ALUPAR-TBO] Nenhuma tabela encontrada.")
                return
            ...
            self.logger.info(f"[ALUPAR-TBO] Processando {len(to_process)} documentos de {target_month}/{target_year}")
            ...
            self.logger.error(f"[ALUPAR-TBO] Erro {agent}: {e}")

    def run(self):
        self.logger.info("Iniciando RialmaIV (TBO Energia / Alupar)...")
